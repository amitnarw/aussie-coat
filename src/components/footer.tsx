    ...
    <footer className="w-full bg-black text-white">
      <div className="container mx-auto px-4 md:px-6 py-16 flex flex-col items-center text-center">
        <div className="mb-8">
          <Link
            href="/"
            className="text-4xl md:text-6xl font-semibold tracking-[2] uppercase"
          >
            CBR Painters
          </Link>
        </div>
        ...
        <nav className="flex items-center justify-center flex-wrap gap-x-4 md:gap-x-8 mb-8">
          {navLinks.map((link, index) => (
            <React.Fragment key={link.name}>
              <Link
                href={link.href}
                className="group text-sm uppercase tracking-wider transition-colors hover:text-gray-300"
              >
        ...
        <div className="text-sm text-center text-gray-300 mb-8">
          <p>ABN 21 652 756 655</p>
          <p>Macgregor, ACT, QLD NSW, Australia</p>
        </div>
